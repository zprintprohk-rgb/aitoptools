...
def patch(path, old, new, count=1):
    ...
    n = s.count(old)
    assert n == count, f"{path}: expected {count} match, got {n} for: {old[:60]!r}"
...
patch(p, """        <p className="byline" style={{ fontSize: '0.85rem', color: 'var(--k-secondary)', margin: '6px 0 0' }}>
        ...
        {(() => {
          ...
          return (
            <div className="related-links" style={{ marginTop: 40 }}>
              ...
              <ul style={{ fontSize: '0.95rem', lineHeight: 1.8, paddingLeft: 20 }}>
                {relComp.map(c => (
                  <li key={c.slug}><Link href={`/compare/${c.slug}/`}>{c.title}</Link> ? head-to-head pricing, quality, and shipping comparison.</li>
                ))}
                {otherHubs.map(x => (
...
# ============ 4. llms.txt: ? 2 ? halloween blog (8/18 PARTIAL ?? P2-5) ============
p = r'public\llms.txt'
